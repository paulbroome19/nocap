...
def test_xml_signature_metric_and_scenario(mini_dpm: Path) -> None:
    ...
    d0, d1 = sig.members
    # DA introduced in release 1 (code 4.1) → eba_dim_4.1:DA ; member eba_BA:x1
    assert (d0.dimension.prefix, d0.dimension.local) == ("eba_dim_4.1", "DA")
    assert d0.dimension.namespace.endswith("/dim/4.1")
    assert (d0.member.prefix, d0.member.local) == ("eba_BA", "x1")
    assert d0.member.namespace.endswith("/dom/BA")
    # MC introduced in release 2 (code 4.2) → eba_dim_4.2:MC ; member eba_MC:x5
    assert (d1.dimension.prefix, d1.dimension.local) == ("eba_dim_4.2", "MC")
